...
# - 이 파일은 로컬 PC가 아니라 GitHub의 클라우드 서버(러너)에서 6시간마다(KST 0/6/12/18시) 실행되므로
#   PC를 꺼놔도 계속 갱신된다
# ==================================================================
...
# '홈쇼핑'/'T커머스'는 원래 ExtendedTopicKeywords(무조건 신뢰)에 있었는데, 이 두 단어는 특정 사업자
# 고유명사가 아니라 일반 용어라 무관한 소비재 기업의 "홈쇼핑 매출" 기사(예: 화장품 브랜드)까지
# 걸러지지 않고 들어오는 문제가 있었음(사용자 피드백) → 아래 제외어 필터가 적용되는 이쪽으로 이동
$BroadKeywords = @('미디어','방송사','콘텐츠','OTT','프로그램','넷플릭스','시청률','엔터테인먼트','홈쇼핑','T커머스')
...
$ExclusionKeywords = @(
    ...
    # 사용자 요청으로 추가: 무관한 소형주가 'OTT향 공급', '콘텐츠 사업 진출' 같은 문구로
    # 증권가 공시/투자 뉴스에 얹혀서 들어오는 경우가 있어(예: 성호전자, 비엠팔텍) 걸러낸다
    '공시','수주','코스닥','상한가','하한가','목표주가','매수의견','투자의견','증권가','상장사',
    # 사용자 요청으로 추가: 식품·화장품 등 소비재 브랜드가 '홈쇼핑 매출', 'IP 콘텐츠 사업'
    # 같은 문구를 써서 걸리는 경우가 있어(예: 농심 '신라면 콘텐츠 사업', 태극제약 '도미나스' 홈쇼핑 매출) 걸러낸다
    '화장품','뷰티','생활건강','라면','식품','제약',
    # 사용자 재신고: '도미나스'는 홈쇼핑 매출 발표를 반복적으로 하는 브랜드라 요약에
    # '화장품'/'생활건강'이 없는 변형 문구로도 계속 걸릴 수 있어 브랜드명 자체를 직접 제외
    '도미나스',
    # 사용자 요청으로 추가: '채널'이 방송 채널이 아니라 "디지털 채널"(유통/마케팅),
    # "유튜브 채널"(브랜드 홍보) 같은 뜻으로 쓰이는 경우도 많아서 걸러낸다.
    # - 삼성전자 '디지털고객경험지수(DCXI)' 가전 순위 기사가 '디지털 채널' 문구로 걸림
    # - '쿠키런' 등 모바일 게임 기사가 "공식 유튜브 채널과 네이버 카페" 같은 상투적
    #   홍보 문구 때문에 '유튜브'+'채널' 조합으로 걸림
    # - '정관장'은 "K-헤리티지 콘텐츠" 같은 문구로 '콘텐츠' 키워드에 걸림
    '디지털고객경험지수','DCXI','쿠키런','정관장'
)
...
function Test-CoreTopicMatch($text) {
    ...
    # KT는 IPTV 3사 중 하나지만 사업 영역이 워낙 넓어(5G, 클라우드, 야구단 등) 이름만으로는 인정 안 함.
    # 유료방송 맥락(IPTV/올레/유료방송/케이블/방송)과 같이 나올 때만 "핵심 유료방송"으로 인정
    if (($text -match '(?<![A-Za-z0-9])KT(?![A-Za-z0-9])') -and
        ($text.Contains('IPTV') -or $text.Contains('올레') -or $text.Contains('유료방송') -or $text.Contains('케이블') -or $text.Contains('방송'))) {
        return $true
    }
    return $false
}
...
function Load-History {
    ...
    try {
        ...
        foreach ($r in @($raw)) {
            ...
            # 제목만 보면 제외어가 요약에만 있는 경우를 놓치므로(사용자 피드백: 도미나스 재발 사례)
            # 제목+요약을 합쳐서 검사한다
            if (-not (Test-TopicMatch "$($r.Title) $($r.Summary)")) { continue }
...
foreach ($q in $NaverQueries) {
    try {
        ...
        for ($i = 0; $i -lt $titleMatches.Count; $i++) {
            ...
            # 사용자 피드백으로 수정: 제목만 검사하면 제외어가 요약(스니펫)에만 있는 경우를 놓침
            # (예: 도미나스 기사 - 제목엔 '화장품'/'생활건강' 언급이 없고 요약에만 있어서 안 걸러짐)
            # RSS 쪽처럼 제목+요약을 합쳐서 검사하도록 통일
            if (Test-TopicMatch "$title $summary") {
...
<div class="wrap" style="margin-top:0; padding-top:0;">
  ...
  <footer>하루 4번(0시/6시/12시/18시) 자동 갱신(GitHub Actions 클라우드에서 실행 · PC와 무관하게 항상 최신 상태) · 최신 뉴스는 최근 4일, 핵심 유료방송/PP 소식은 최근 7일간 누적 · 최대 10페이지까지 보관<br>🔥 핵심이슈 배지 = 대가산정 · 채널평가 · 채널퇴출 · 사용료 · 실적 관련 기사 (배지에 구체적 이슈 표시) · 새로고침 버튼은 마지막으로 저장된 최신본을 다시 불러옵니다.</footer>
</div>
<script>
function doRefresh() {
